...
export default function EditQuoteForm({ quote }: EditQuoteFormProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([getBusinessProfile(), getWorkspace()])
      .then(([bp, ws]) => {
        setBusinessName(bp?.businessName || ws.name || 'Grupo Movensa');
      })
      .catch(() => {});
  }, []);
  ...
  const initialDiscountType =
    quote.totals.discountType === 'percentage' ? ('percent' as const) : ('flat' as const);
  ...
  const [signatureRequired, setSignatureRequired] = useState(
    quote.settings.requireSignature || false
  );
  ...
  const handleSubmit = async (isDraft: boolean) => {
    ...
    try {
      ...
      const result = await updateQuote(quote.id, {
        title: lineItems[0]?.name || quote.title || 'Cotización sin título',
        currency,
        blocks,
        notes,
        terms,
      ...
      if (result.success) {
        if (!isDraft) {
          ...
          if (!sendResult.success) {
            toast.error(
              sendResult.error ||
                'La cotización se guardó, pero el correo no pudo enviarse. Puede reenviarlo desde la lista de cotizaciones.'
            );
            router.push('/quotes');
            return;
          }
          toast.success(`Cotización ${quoteNumber} actualizada y enviada correctamente`);
        } else {
          toast.success(`Cotización ${quoteNumber} actualizada correctamente`);
        }
        router.push('/quotes');
      } else {
        toast.error(result.error || 'No se pudo actualizar la cotización');
      }
    } catch (err) {
      console.error('Update quote error:', err);
      toast.error('No se pudo actualizar la cotización. Revise los datos e inténtelo nuevamente.');
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className="pb-8">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input value={quoteNumber} className="h-11" disabled />
                  </div>
                  <div className="space-y-1.5">
